namespace Klondike.LevelGeneration {
    ...
    /// <summary>
    /// 沿求解序列重放时得到的动态指标；累计步数与 <see cref="Klondike.Entities.Board.MovesMade"/> 同源（每步贡献为重放前后 <c>MovesMade</c> 差值）。
    /// </summary>
    public struct DealReplayMetrics {
        /// <summary>
        /// 首次出现「桌面盖牌张数减少」（有盖牌由暗变明）时的累计执行序列步数（含本步），即该时刻的 <see cref="Klondike.Entities.Board.MovesMade"/>。未发生则为 -1。
        /// </summary>
        public int FirstRevealStepTotal;

        /// <summary>
        /// 同上时刻：各步对 <see cref="Klondike.Entities.Board.MovesMade"/> 的增量中，源摞非 <see cref="Klondike.Entities.Board.kWastePile"/> 的增量之和（含桌面列、回收位为源；开启 <see cref="Klondike.Entities.Board.AllowFoundationToTableau"/> 时回收→桌面亦在此列）。
        /// </summary>
        public int FirstRevealTableauSteps;

        /// <summary>
        /// 同上时刻：各步对 <see cref="Klondike.Entities.Board.MovesMade"/> 的增量中，源摞为 <see cref="Klondike.Entities.Board.kWastePile"/> 的增量之和。
        /// </summary>
        public int FirstRevealTalonSteps;

        /// <summary>
        /// 七列均无盖牌（<see cref="Klondike.Entities.Pile.First"/>==0 或列空）时的累计执行序列步数（该时刻 <see cref="Klondike.Entities.Board.MovesMade"/>）；未发生则为 -1。
        /// </summary>
